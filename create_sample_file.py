...
class DataTreeMaker:
    def __init__(self, tstart, tend, lon_min, lon_max, lat_min, lat_max, grid_resol, void_timestamps):
    ...
    def get_time_bounds(self):
        time_bounds = self._get_bounds(self.time.values, resol=np.timedelta64(1, "D"), align="left")
    ...
    def get_lon_bounds(self):
        lon_bounds = self._get_bounds(self.lon, self.grid_resol, align="center").astype("float64")
    ...
    def get_lat_bounds(self):
        lat_bounds = self._get_bounds(self.lat, self.grid_resol, align="center").astype("float64")
...
class Clouds:
    ...
    def _get_cfc(self):
        ...
        for t in range(self.time.size):
            lon_term = np.sin(t/5.*np.deg2rad(self.mlon))**2
            lat_term = np.cos(t/10.*np.deg2rad(self.mlat))**2
    ...
    def _get_nobs(self):
        ...
        tmax = max(times)
        for t in times:
            nobs[t, :, :] = 96 * np.sin(t/5.*np.deg2rad(self.mlon + (lon_max-lon_min)/2.0*t/float(tmax))*np.deg2rad(self.mlat))**2
...
class Radiation:
    ...
    def get_sdu(self):
        ...
        return xr.DataArray(hearts, dims=("time", "lat", "lon"), attrs={
